...
export default function RegisterForm() {
  ...
  return (
    <form action={formAction} className="flex max-w-[300px] flex-col gap-2">
      ...
      <div className="flex flex-col gap-2">
        <input className="p-1 rounded-sm" name="email" type="email" placeholder="Email" />
      </div>
      {state?.errors?.email && (
...
function SubmitButton() {
  ...
  return (
    <button
      disabled={pending}
      type="submit"
      className="bg-black rounded-md mt-6 mx-24 py-2 transition delay-0 duration-300 ease-in-out hover:-translate-y-1 hover:scale-110 hover:bg-lime-300 hover:text-black"
    >
      Sign Up
    </button>
  );
